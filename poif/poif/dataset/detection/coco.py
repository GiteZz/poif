...
from poif.dataset.detection.base import DetectionFileOutputFormat, DetectionDataset
...
class CocoDetectionDataset(DetectionDataset):
    ...
    def parse_annotation_file(self, dataset_type: str, inputs: List[TaggedData]):
        ...
        for image_info in annotation_data['images']:
            ...
            if self.data_folders[dataset_type] != '':
                original_filename += '/'
            original_filename += f'{image_info["file_name"]}'

            if original_filename in mapping:
                new_inputs[img_id] = DetectionInput(data=mapping[original_filename],
        ...
        if not_found_count > 0:
            print(f'WARNING: {not_found_count} out of {len(annotation_data["images"])} were not found on disk but '
        ...
        for annotation in annotation_data['annotations']:
            ...
            current_input.add_bounding_box(BoundingBox(label=category_id,
