...
def get_readme_sequence(expected_result: ReadmeConfig = None):
    if expected_result is None:
        ...
        expected_result = ReadmeConfig(enable=True, enable_filetree=True, enable_image_gallery=True, image_remote=remote_config)
    else:
        remote_sequence, remote_config = get_remote_config_sequence(expected_result.image_remote)

    readme_sequence = [bool_to_yes_no(expected_result.enable),
...
def get_cache_sequence(expected_result: CacheConfig = None):
    if expected_result is None:
        expected_result = CacheConfig(enable=True, data_storage_location=Path.home(), git_storage_location=Path.home(), cache_uploads=True)

    sequence = [bool_to_yes_no(expected_result.enable),
...
def get_collection_sequence(expected_result: DataCollectionConfig = None):
    if expected_result is None:
        remote_sequence, remote_config = get_remote_config_sequence()
        expected_result = DataCollectionConfig(name='name', folders=['val', 'test'], files=['01.jpg'], data_remote=remote_config)
    else:
