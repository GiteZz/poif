...
class ReadmeConfig(Config):
    ...
    def prompt(cls, default_readme: "ReadmeConfig" = None) -> "ReadmeConfig":
        ...
        default_enable_filetree = (
            None if default_readme is None else default_readme.enable_filetree
        )
        default_enable_gallery = (
            None if default_readme is None else default_readme.enable_image_gallery
        )

        enable = yes_with_question("Create readme?", default=default_enable)
        if not enable:
            return ReadmeConfig(
                enable=False, enable_filetree=False, enable_image_gallery=False
            )

        enable_filetree = yes_with_question(
            "Add filetree?", default=default_enable_filetree
        )
        enable_image_gallery = yes_with_question(
            "Add image galley?", default=default_enable_gallery
        )
