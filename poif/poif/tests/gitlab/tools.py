...
from poif.tests.gitlab.start import GitlabConfig
...
def create_repo(config: GitlabConfig, name) -> str:
    ...
    return config.url + '/root/' + create_repo_param['path']
...
def execute_gitlab_api_call(api_route: str, config: GitlabConfig, params=None, method: RequestType = RequestType.GET):
...
def headers_from_config(config: GitlabConfig):
    return {
        'PRIVATE-TOKEN': config.root_api_key
    }
...
# print(create_repo(GitlabConfig(), 'test4'))
