...
def get_repo_remotes_from_config(
    config: MinioConfig,
) -> Tuple[RemoteConfig, RemoteConfig]:
    data_config = S3Config(
        url=f"http://localhost:{config.port}", bucket="datasets", profile="datasets"
    )
    readme_config = S3Config(
        url=f"http://localhost:{config.port}",
        bucket="readme-images",
        profile="datasets",
    )

    data_remote = RemoteConfig(
        remote_type=RemoteType.S3, data_folder="data", config=data_config
    )
    readme_remote = RemoteConfig(
        remote_type=RemoteType.S3, data_folder="data", config=readme_config
    )
