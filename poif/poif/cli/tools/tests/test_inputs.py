...
def test_monkey_patch_sequence(monkeypatch):
    ...
    monkeypatch.setattr("builtins.object", input_returns)
...
def test_input_with_possible_default(monkeypatch):
    monkeypatch.setattr("builtins.object", lambda: "")
    assert input_with_possible_default(default="tree") == "tree"

    monkeypatch.setattr("builtins.object", lambda: "desk")
...
def test_yes(monkeypatch):
    ...
    for valid_yes in valid_yesses:
        monkeypatch.setattr("builtins.object", lambda: valid_yes)
        assert yes()

    valid_noos = ["n", "no", "NO", "N", "No"]
    for valid_no in valid_noos:
        monkeypatch.setattr("builtins.object", lambda: valid_no)
        assert not yes()
