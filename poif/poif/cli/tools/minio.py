...
def upload_datasets_images(s3_config: S3Remote, files: List[Tuple[Path, Path]]):
    ...
    dataset_sess = boto3.session.Session(profile_name=s3_config.config.profile)
    s3 = dataset_sess.resource("s3", endpoint_url=s3_config.config.endpoint, config=Config(signature_version="s3v4"))
    # Rescale the images
